...
| "Resume" / "continue" / "you can move again" / "release" | `release_hold()` | B ends the indefinite hold and resumes autonomous parcel work. |
...
| **L3** "Move both agents near (x,y) within distance 3 and wait. 500pts" | `order_partner_goto` + `go_to` + `halt_partner` + `hold()` | Two different tiles within range are chosen; A and B park there and hold for each other. |
...
| **L3** "Red light, green light: move to an odd row and wait for our message. 700pts" | `Final Answer` (acknowledge) | B just acknowledges; the runtime freezes both agents on the RED LIGHT shout and resumes them on GREEN LIGHT automatically. |
