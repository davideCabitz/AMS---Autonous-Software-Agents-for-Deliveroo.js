...
| "Go to (x,y)" / "move to (x,y)" / "go to the leftmost tile" | `go_to(x,y)` | B path-finds to the tile and stops there; returns on arrival or a failure reason. Accepts a side keyword (`leftmost`/`rightmost`/`top`/`bottom`), resolved to the nearest reachable edge tile. |
...
| "Go to (x,y) and drop it" / "drop it in the leftmost tile" | `go_put_down(x,y or leftmost/rightmost/top/bottom)` | B navigates to the tile **and** drops there in one tool call (the go+drop sibling of `go_pickup`/`deliver`; reuses the same drop logic as `put_down`). Accepts coordinates or a side keyword (resolved to the nearest reachable edge tile). Scores only on a delivery tile; pick up first if carrying nothing. |
| "Pick up a parcel, then go to X and drop it" | `pickup_next_parcel()` → `go_put_down(X)` | B executes the sub-tasks in order, in one directive. `go_put_down` navigates to X and drops there in a single tool call. Passing through X mid-pickup does not satisfy the later step. |
| "Go to (x,y) and wait" / "go there and hold" | `go_to_stay(x,y)` → `hold()` (or `wait(N)` for a timed wait) | B navigates and stays parked at (x,y), then holds/waits *there*. Use `go_to_stay` (not `go_to`) for any "go to X and \<wait/hold/deliver\>" composite (for "…and drop it", use `go_put_down`). Also accepts a side keyword. |
...
| **L1** "Drop a package in the leftmost tile to get 5pt" | `go_put_down(leftmost)` | POSITIVE one-shot **drop** bonus: B navigates to the leftmost tile and drops there in one call — `go_put_down` resolves the side keyword to the nearest reachable edge tile, replacing the old `get_map_info` → `go_to_stay` → `put_down` chain. Pick up first if carrying nothing. The leftmost tile is NOT necessarily a delivery tile — `go_put_down`/`put_down`, never `deliver()`. NOT `forbid_delivery`. |
...
| **L3** negative light offer: "Red light green light: move to an odd row and wait. -300pts" | `start_light_mission("pts=-300")` | The signed reward is **always** routed through the tool — never a bare `Mission declined.`. It adds to `lightNet`; since the net is < 0 the tool itself returns `Mission declined.` and disarms (no `hold_on_parity`). A later "…200pts" → `start_light_mission("pts=200")` nets `-100` → still `Mission declined.`. Routing every offer (positive *and* negative) through the tool is what lets the light offers accumulate, so a bare Final-Answer decline must not be used. |
...
| **L3** bare "red light" / "green light" (just the colour words, no imperative, not an announcement) | — (classified `IGNORE`) | Dropped before any handling: it never arms, stops, or resumes the agents, and gets no reply — even mid-game. Only the full live shouts and the announcements above trigger anything. |
