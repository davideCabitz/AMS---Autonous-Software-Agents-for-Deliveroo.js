...
export class StrategyHurry extends StrategyGreedy {
    ...
    /** @type {AntiLockExplorer} Commit/stall/blacklist machinery */
    #explorer = new AntiLockExplorer(log);
    ...
    exploreIfIdle(currentIntent) {
        ...
        this.#explorer.trackMovement(now);
        ...
        this.#explorer.expireBlacklist(now);
        ...
        if (currentIntent && currentIntent[0] === 'go_explore') {
            ...
            const key = `${tx}_${ty}`;
            const { stalled, timedOut } = this.#explorer.commitStatus(key, now);
            const observed = this.#visited.has(key);

            if (!observed && !stalled && !timedOut) return null; // keep heading to it

            if (stalled || timedOut) this.#explorer.giveUp(key, now, stalled);
            else this.#explorer.clearCommit();
        }
        ...
        if (target) {
            this.#explorer.commitTo(`${target.x}_${target.y}`, now);
