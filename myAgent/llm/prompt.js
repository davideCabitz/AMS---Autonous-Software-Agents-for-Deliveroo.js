...
export function buildSystemPrompt(objective) {
    ...
    return [
        ...
        '- NEGATIVE or REDUCED-reward offers: if PERFORMING the requested action pays negative points',
        '  ("for -10 pts", "you lose N") or only a reduced FRACTION of the normal reward ("0.3 of the',
        '  standard reward", "X% of normal", "less reward than usual"), it loses/wastes points — DECLINE',
        '  with EXACTLY "Mission declined." and change nothing. (Different from a PENALTY mission above,',
        '  which you ACCEPT as an avoid-constraint, and from a positive bonus with a threshold like',
        '  "deliver total reward <= 10 for a bonus", which you ACCEPT.)',
        '',
        ...
        '- "Every time you deliver in (x,y) you get N× pts" (N>1) -> apply_mission {"deliveryMultipliers":[[x,y,N],...]}.',
        ...
        '- "Move to an odd-numbered row and wait for our message" (a red/green-light variant) ->',
        '  a "row" is the y coordinate, so an ODD row = odd y. get_my_position; if your y is even,',
        '  go_to a reachable tile with odd y (e.g. x,y+1 or x,y-1); also order_partner_goto an',
        '  odd-row tile for the partner; then acknowledge — the RED/GREEN reflex handles stop/go.',
        ...
        '    "deliveryMultipliers": [[x,y,mult],…] — per-tile delivery reward scale: 5 for "5× pts in',
        '      (x,y)", 0 for "0 pts / no reward in (x,y)". Agent prefers high-× tiles and avoids 0×.',
